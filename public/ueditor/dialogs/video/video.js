...
(function(){
    ...
    function initVideo(){
        ...
        addUrlChangeListener($G("videoUrl"),$G("videoCover"));
        addOkListener();
        ...
        (function(){
            ...
            if(img && img.className){
                ...
                if(hasFakedClass || hasUploadClass) {
					...
					$G("videoCover").value = cover = img.getAttribute("data-cover");
    ...
    function insertSingle(){
        var width = $G("videoWidth"),
			...
			cover=$G('videoCover').value,
            align = findFocus("videoFloat","name");
        if(!url) return false;
        if ( !checkNum( [width, height] ) ) return false;
        editor.execCommand('insertvideo', {
            url: convert_url(url),
			dataCover: convert_url(cover),
            width: width.value,
            height: height.value,
            align: align
     ...
     * @param url
     */
    function addUrlChangeListener(url,cover){
        if (browser.ie) {
            url.onpropertychange = function () {
                createPreviewVideo( this.value ,null);
            }
			cover.onpropertychange = function () {
                createPreviewVideo( null,this.value );
            }
        } else {
     ...
     * @param url
     */
    function createPreviewVideo(url,cover){
        if ( !url && !cover )return;

		var url=$("#videoUrl").val();
		var cover=$("#videoCover").val();
		var conUrl = convert_url(url);
		conUrl = utils.unhtmlForUrl(conUrl);
		var conCover = convert_url(cover);
		conCover = utils.unhtmlForUrl(conCover);
