...
class NavShow extends Model
{
    ...
    {
        $dataList = Db::name('nav_show')->order("sort desc,id DESC")->select();
    ...
    public function add()
    {
        ...
        if(true == Db::name("nav_show")->insert($data))
        {
    ...
    public function edit()
    {
        ...
        if(true == Db::name("nav_show")->where(['id'=>$post['id']])->update($data))
        {
            exit(json_encode(['code'=>1,'msg'=>'操作成功']));
        } 
        exit(json_encode(['code'=>0,'msg'=>'操作失败']));
    ...
    {
        $dataInfo = Db::name("nav_show")->where(['id'=>$id])->find();
        return $dataInfo;
    }
    /**
     * [infoDelete 删除]
    ...
    {
        if(true == Db::name("nav_show")->where(['pid'=>$id])->count())
        {
            exit(json_encode(['code'=>0,'msg'=>'请先删除当前分类下的子分类']));
        }
        if(true == Db::name("nav_show")->where(['id'=>$id])->delete())
        {
