<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:65:"/www/users/HA611975/WEB/application/admin/view/cases/publish.html";i:1570586669;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body class="childrenBody">
    <form class="layui-form layui-form-pane" id="form_array">
        ...
        <div class="layui-upload" id="img">
            ...
            <button type="button" class="layui-btn" id="cases_click">上传图片</button>
            <div class="layui-upload-list">
                <label class="layui-form-label" style="border: 1px solid #fff;"></label>
                <img class="layui-upload-img" id="cases_img" width="150" height="150" <?php if(!(empty($getInfo['img']) || ($getInfo['img'] instanceof \think\Collection && $getInfo['img']->isEmpty()))): ?> src="<?php echo citrixGetUrl($getInfo['img']); ?>" <?php endif; ?>>
                <input type="hidden" name="img" id="cases_thumb" <?php if(!(empty($getInfo['img']) || ($getInfo['img'] instanceof \think\Collection && $getInfo['img']->isEmpty()))): ?> value="<?php echo $getInfo['img']; ?>"<?php endif; ?>>
            </div>
        </div>
        <div class="layui-upload" id="img">
            <label class="layui-form-label">二维码</label>
            <button type="button" class="layui-btn" id="code_click">上传二维码</button>
            <div class="layui-upload-list">
                <label class="layui-form-label" style="border: 1px solid #fff;"></label>
                <img class="layui-upload-img" id="code_img" width="150" height="150" <?php if(!(empty($getInfo['img']) || ($getInfo['img'] instanceof \think\Collection && $getInfo['img']->isEmpty()))): ?> src="<?php echo citrixGetUrl($getInfo['code_img']); ?>" <?php endif; ?>>
                <input type="hidden" name="code_img" id="code_thumb" <?php if(!(empty($getInfo['code_img']) || ($getInfo['code_img'] instanceof \think\Collection && $getInfo['code_img']->isEmpty()))): ?> value="<?php echo $getInfo['code_img']; ?>"<?php endif; ?>>
            </div>
        </div>
        ...
        <div class="layui-form-item layui-form-text">
            ...
            <div class="layui-input-block" >
                <textarea placeholder="请输入案例内容" name="content" id="container"><?php if(!(empty($getInfo['content']) || ($getInfo['content'] instanceof \think\Collection && $getInfo['content']->isEmpty()))): ?><?php echo htmlentities($getInfo['content']); endif; ?></textarea>
            </div>
        </div>
    ...
    <script type="text/javascript">
        layui.use(['form','upload'], function()
        {
            ...
            //上传图片
            upload.render({
                elem: '#cases_click' //绑定元素
                ,url: "<?php echo url('admin/common/upload'); ?>" //上传接口
                ,data:{use:'cases_default'}
                ,done: function(res)
                {
                    $("#cases_img").attr('src',res.src);
                    $("#cases_thumb").val(res.id);
                }
                ,error: function()
                {
            ...
            //上传二维码
            upload.render({
                elem: '#code_click' //绑定元素
                ,url: "<?php echo url('admin/common/upload'); ?>" //上传接口
                ,data:{use:'code_default'}
                ,done: function(res)
                {
                    $("#code_img").attr('src',res.src);
                    $("#code_thumb").val(res.id);
                }
                ,error: function()
                {
                    layer.alert("Systematic anomaly！",{icon:2});
            ...
            form.on("submit(form_submit)",function(data){
                $.ajax({
                    url:"<?php echo url('admin/cases/publish'); ?>",
